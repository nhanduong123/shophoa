<?php
	...
	if(isset($_POST["xoadonhang"]))
	{
		...
		if (isset($_POST["madh"]))
			$madh = $_POST["madh"];
		$query="select * from donhang where donhang.madh=madh";
		$data=$loai->queryXuatdonhang($query, $madh);
		//print_r($data);
		
		function ktdh($madh1){
			$loai = new db();
			$query="select * from donhang where donhang.madh=madh";
			$data=$loai->queryXuatdonhang($query, $madh1);
			...
			if(ktdh($madh,$data)==false)
				...
				$query=	"Delete From donhang Where madh like :madh";
				$data=$loai->queryXoadh($query,$madh);
				?>
				<script type="text/javascript">
...
<div class="container">
      ...
      <form class="form-dk" action="xoadonhang.php" method="post">
        <h2 class="form-signin-heading">Xóa Đơn Hàng</h2>
        <label for="madh" class="sr-only">Mã đơn hàng</label>
        <input type="text" id="madh" class="form-control" name="madh" placeholder="Mã đơn hàng" required>
        <label for="lydo" class="sr-only">Lý do</label>
        <input type="text" id="lydo" class="form-control" placeholder="Lý do" required autofocus>
        <button class="btn btn-lg btn-primary btn-block" name="xoadonhang" type="submit">Xóa Đơn Hàng</button>
      </form>
